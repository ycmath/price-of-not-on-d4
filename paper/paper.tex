...
\author{Won Chul Yang\\ \small Seoul, Republic of Korea \\ \small \texttt{wcy0969@gmail.com}}
...
Boolean transport route is discussed at Theorem~\ref{thm:exact}. For the
surrounding negation-limited literature --- the Markov-style upper-bound
argument \cite{fischer}, constant-depth circuits \cite{santha},
bounded-depth circuits \cite{sung}, lower bounds against few negations
\cite{amano}, negation-limited formula structure \cite{guo}, and
multi-valued circuit-model laws \cite{kochergin} --- the formula-exact
multi-valued law proved here appears to be new.
...
\begin{proof}
The lower bound $\mcr(f)\le\nu(f)$ is Proposition~\ref{prop:D4}:
projections contribute no chain reversal (Lemma~\ref{lem:D1}), $\meetR$
and $\joinR$ are reversal-subadditive (Lemma~\ref{lem:D2}), and one
occurrence of $\sigma$ contributes at most one new reversal along any
chain (Lemma~\ref{lem:D3}). For the upper bound,
Theorem~\ref{thm:WB} gives the containment $W_f\subseteq B_f$
unconditionally, so the canonical shell
$S_f=A_f\cap{\downarrow}W_f$ is an $A_f$-ideal with
$W_f\subseteq S_f\subseteq B_f$ (Corollary~\ref{cor:D12});
Proposition~\ref{prop:D5} realizes $\chi_{S_f}$ with one occurrence of
$\sigma$, Theorem~\ref{thm:decrement} gives $\mcr(f_{S_f})=\mcr(f)-1$,
and the induction of Theorem~\ref{thm:D14} yields $\nu(f)\le\mcr(f)$. The
exact-equality corollary of Appendix~D assembles
$\nu(f)=\mcr(f)=\dec(f)$. Every cited result is proved in full in
Appendix~D.
\end{proof}
...
ICALP 2009, LNCS 5555, 701--712. doi:10.1007/978-3-642-02927-1\_58.
(Preliminary version: arXiv:0811.0699, 2008.)
\bibitem{bennett} C.~H. Bennett. \emph{Logical reversibility of
computation.} IBM J.~Res.~Dev. 17(6):525--532, 1973. doi:10.1147/rd.176.0525.
...
\bibitem{fischer} M.~J. Fischer. \emph{Lectures on network complexity.}
Technical Report 1104, Yale University, 1974 (revised 1996).
\bibitem{santha} M.~Santha, C.~Wilson. \emph{Limiting negations in
constant depth circuits.} SIAM J.~Comput. 22(2):294--302, 1993.
doi:10.1137/0222022.
\bibitem{sung} S.~C. Sung, K.~Tanaka. \emph{Limiting negations in
bounded-depth circuits: an extension of Markov's theorem.} ISAAC 2003,
LNCS 2906, 108--116. doi:10.1007/978-3-540-24587-2\_13.
\bibitem{amano} K.~Amano, A.~Maruoka. \emph{A superpolynomial lower bound
for a circuit computing the clique function with at most
$(1/6)\log\log n$ negation gates.} SIAM J.~Comput. 35(1):201--216, 2005.
doi:10.1137/S0097539701396959.
\bibitem{guo} S.~Guo, I.~Komargodski. \emph{Negation-limited formulas.}
APPROX/RANDOM 2015, LIPIcs. doi:10.4230/LIPIcs.APPROX-RANDOM.2015.850.
\bibitem{kochergin} V.~V. Kochergin, A.~V. Mikhailovich. \emph{The minimum
number of negations in circuits for systems of multi-valued functions.}
Discrete Mathematics and Applications, 2017. doi:10.1515/dma-2017-0030.
(Preliminary version: arXiv:1510.05942.)
\end{thebibliography}

\section*{Authorship \& provenance}

Won Chul Yang, independent researcher. \textbf{This paper is the author's
own research}: the mathematical content --- the definitions, theorems, and
proofs, including the original preprint that this edition revises --- is
the author's original work. AI assistance (Anthropic Claude family) was
used \textbf{only for the machine-verification layer}: the Lean~4
mechanization and the exhaustive machine audit/replay of the author's
proofs, with the Lean~4 kernel as the acceptance gate for that layer. The
interval single-regime simplification (Theorem~\ref{thm:WB}) emerged from
that machine audit of the author's Appendix~D and was adopted by the
author. This division of labor differs from the author's two earlier
AI-collaborative releases (\texttt{cohomological-price-of-not} and
\texttt{inversion-wilf-spi}), which were produced in an AI research loop
and are labeled accordingly. No claim of academic priority is made beyond
full disclosure of how the work was produced. Corrections are invited.
